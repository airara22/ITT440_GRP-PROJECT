...
def threaded_client(conn):
   conn.send(("\n\t\t Dear HEBIGēMU players, ready to play?").encode('utf-8')) #send opening message to client
   while True:
...
while True:
   ...
   print("\n\t\tConnected successfully to: " + addr[0] + ':' + str(addr[1]))
   start_new_thread(threaded_client, (client, ))
   threadCount += 1
   print('\t\tNumber of Players Connected to Server: ' + str (threadCount))
s.close()
